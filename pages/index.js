...
export default function HomePage() {
  const [whatIsShown, setWhatIsShown] = useState({ introIsShown: true, mainIsShown: false, newDesign: false, moreOptions: false, highscoreIsShown: false, setInfoIsShown: false, resultIsShown: false });
  const { introIsShown, mainIsShown, highscoreIsShown, setInfoIsShown, resultIsShown, moreOptions } = whatIsShown;
  ...
  const { gameMode, numberOfPlayers, nameOfPlayer1, nameOfPlayer2, cardRows, numberDealt, cardSet, shuffle, delayTime, typeOfSet, size, timerWanted } = options;
  const [activePlayer, setActivePlayer] = useState(nameOfPlayer1);
  const zeroPoints = [{ name: nameOfPlayer1, points: 0 }, { name: nameOfPlayer2, points: 0 },]
  ...
  const cardColumns = Math.ceil(numberDealt / 4);
  const cardSectionHeight = windowHeight - 54;
  const cardHeight = cardSectionHeight / 4 - 6;
  const shiftRight = cardSectionHeight / 8 + 1;
  const moreColumns = cardColumns - 4;
  const upperWidth = `${cardSectionHeight + moreColumns * (shiftRight * 2)}px`;
  ...
  return (
    <>
      ...
      <UpperSection $maxwidth={overallMaxwidth} $upperWidth={upperWidth}>
          ...
          <StatLine>Round: {roundCount}</StatLine>
         
        </UpperSection>
      
   <NewSquareSection $height={cardSectionHeight} $addColumns={cardColumns - 4} $fraction="1fr " $shiftRight={shiftRight * (cardColumns - 4)}>
        <FirstSquare $height={cardHeight}>
        <ControlsSection>
            <FlexRowWrapper>
              <StandardButton $width={cardHeight / 5} onClick={handlePlayButton}>{!running || gameIsPaused ? "►" : "‖"}</StandardButton>
            ...
            <GameOptionsV2 options={options} cardHeight={cardHeight} onUpdateOptions={updateOptions} />
        
          <FlexRowWrapper>
            <MinimalDisplay> Points: {scores[0].points} 
            </MinimalDisplay>      
            <MinimalDisplay> Cards: {numberDealt} 
            </MinimalDisplay>
            </FlexRowWrapper>
            <FlexRowWrapper>
        ...
        {running === true ? (squareState.map((square, index) =>
          ...
          {numberOfPlayers > 1 &&
            <FirstSquare $height={cardHeight}>
          <StatLine> <> {nameOfPlayer2}  {activePlayer === nameOfPlayer2 && "(active)"}<br />
